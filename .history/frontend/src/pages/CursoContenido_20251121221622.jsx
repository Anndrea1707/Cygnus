...
export default function CursoContenido({ curso, onNavigate, onFinalizarCurso }) {
    ...
    const guardarProgreso = async (modulo = moduloActual, contenido = contenidoActual) => {
        ...
        if (!usuarioLS || !curso?.id) {
            console.error("❌ No hay usuario o curso ID");
            return;
        }

        const totalModulos = curso.modulos.length;
        const totalContenidosModulo = curso.modulos[modulo]?.contenido?.length || 1;

        console.log("📊 Guardando progreso:", {
            usuarioId: usuarioLS._id,
            cursoId: curso.id, // ✅ Asegurar que se envía el cursoId
            moduloActual: modulo,
        ...
        try {
            const response = await fetch("http://localhost:4000/api/progreso/contenido", {
                ...
                body: JSON.stringify({
                    usuarioId: usuarioLS._id,
                    cursoId: curso.id, // ✅ Esto es crucial
                    moduloActual: modulo,
                    contenidoActual: contenido,
                    totalModulos: totalModulos,
                    totalContenidosModulo: totalContenidosModulo
                })
            });
            ...
            if (data.success) {
                ...
                console.error("❌ Error guardando progreso:", data.error);
            }
        } catch (error) {
