...
export default function CursoVista({ onNavigate, curso }) {
  ...
  // ⭐ Cantidad de preguntas visibles según reglas:
  // - Evaluación de módulo → máximo 10, si tiene menos mostrar cantidad real
  // - Evaluación final → máximo 15, si tiene menos mostrar cantidad real
  const obtenerPreguntasVisibles = (evaluacion, esFinal = false) => {
    if (!evaluacion?.preguntas) return 0;

    const total = evaluacion.preguntas.length;

    if (esFinal) {
      return total < 15 ? total : 15;
    }
  ...
  const normalizarModulosCompletados = (modulosCompletados = []) => {
    ...
    for (const m of modulosCompletados) {
      ...
      map.set(m.moduloIndex, {
        ...
        aprobado: estaCompletado // ✅ Nueva propiedad para claridad
      });
    }
    ...
    const cargarProgreso = async () => {
      ...
      try {
        ...
        if (data.success && data.progreso) {
          // Normalizar modulosCompletados por seguridad - SOLO completados si aprobaron
          ...
          // MOSTRAR MODAL SOLO SI realmente está COMPLETADO (aprobó evaluación final)
          if (progresoNormalizado.cursoCompletado && progresoNormalizado.notaEvaluacionFinal >= 70) {
  ...
  // Devuelve true si todos los módulos están completados Y APROBADOS (nota >= 70)
  const todosLosModulosCompletos = () => {
    ...
    const completados = (progresoCurso.modulosCompletados || []).filter(m => m.completado && m.aprobado);
  ...
  const soloFaltaEvaluacionFinal = () => {
    ...
    // todos los módulos completos Y APROBADOS y la evaluación final NO completada O no aprobada
    return todosLosModulosCompletos() &&
      (!progresoCurso.evaluacionFinalCompletada || progresoCurso.notaEvaluacionFinal < 70);
  };
  ...
  const handleEmpezarCurso = () => {
    ...
    // Si el curso está completado Y APROBADO, mostrar modal completado
    if (progresoCurso && progresoCurso.cursoCompletado && progresoCurso.notaEvaluacionFinal >= 70) {
      setMostrarModalCompletado(true);
      return;
    }
  ...
  // ✅ CORREGIDO: Solo permite avanzar si el módulo anterior está COMPLETADO Y APROBADO
  const moduloBloqueado = (moduloIndex) => {
    ...
    // Siguiente módulo solo permitido si el actual está completado Y APROBADO
    const actualCompletado = completados.some(m =>
      Number(m.moduloIndex) === Number(actual) &&
      m.completado === true &&
      m.aprobado === true
    );
                    ...
                    {modulo.tieneEvaluacion && (
                      <div className="progreso-nota">
                        <div
                          ...
                          style={{ width: `${Math.min(modulo.notaEvaluacion, 100)}%` }}
                        ></div>
                      </div>
                    )}
                ...
                <div className="nota-final">
                  ...
                  <div className="progreso-nota">
                    <div
                      ...
                      style={{ width: `${Math.min(notas.evaluacionFinal.nota, 100)}%` }}
                    ></div>
                  </div>
                  <div className="estado-final">
                    {notas.evaluacionFinal.nota >= 70 ? (
                      ...
                      <span className="estado reprobado">❌ Curso Reprobado - Debes volver a intentarlo</span>
                    )}
                  </div>
                </div>
              </div>
  ...
  // calcular número único de módulos completados Y APROBADOS para mostrar en UI
  const contadorModulosCompletados = (progresoCurso?.modulosCompletados || [])
    .filter(m => m.completado && m.aprobado).length;
          ...
          <div className="sidebar-contenido">
            ...
            <div className="curso-info-resumen">
                ...
                <div className="progreso-global">
                  ...
                  <div className="progreso-detalles">
                    ...
                    {contadorModulosCompletados > 0 && (
                      <div className="progreso-item">
                        <span className="progreso-label">Módulos aprobados:</span>
                        <span className="progreso-valor">{contadorModulosCompletados}</span>
                      </div>
                    )}
                    {progresoCurso.evaluacionFinalCompletada && (
                      <div className={`progreso-item ${progresoCurso.notaEvaluacionFinal >= 70 ? 'completado' : 'reprobado'}`}>
                        <span className="progreso-label">
                          {progresoCurso.notaEvaluacionFinal >= 70 ? '✅' : '❌'} Evaluación final:
                        </span>
                        <span className="progreso-valor">
                          {progresoCurso.notaEvaluacionFinal >= 70 ? 'Aprobada' : 'Reprobada'}
                        </span>
                      </div>
                    )}
                  </div>
              ...
              >
                ...
                ) : progresoCurso && progresoCurso.cursoCompletado && progresoCurso.notaEvaluacionFinal >= 70 ? (
                        ...
                        <div className="evaluacion-contenido">
                          ...
                          <div className="evaluacion-meta">
                            <span className="preguntas-count">
                              {obtenerPreguntasVisibles(modulo.evaluacion, false)} preguntas
                            </span>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="evaluacion-final-info">
                    ...
                    <div className="evaluacion-final-meta">
                      <span className="preguntas-count final">
                        {obtenerPreguntasVisibles(cursoActual.evaluacionFinal, true)} preguntas
                      </span>
                    </div>
                  </div>
                </div>
