...
export default function CursoVista({ onNavigate, curso }) {
  ...
  const [usuario, setUsuario] = useState(null); // ✅ Agregar estado para usuario
  ...
  useEffect(() => {
    const cargarProgreso = async () => {
        if (!usuario || !cursoActual?.id) {
            console.log("❌ No hay usuario o curso ID para cargar progreso");
            return;
        }
        ...
        try {
            ...
            const response = await fetch(`http://localhost:4000/api/progreso/curso/${usuario._id}/${cursoActual.id}`);
    ...
    cargarProgreso();
}, [usuario, cursoActual?.id]); // ✅ Usar cursoActual.id como dependencia
  ...
  const handleEmpezarCurso = () => {
    ...
    onNavigate("curso-contenido", {
      curso: cursoActual,
      moduloIndex: progresoCurso?.moduloActual || 0,
      contenidoIndex: progresoCurso?.contenidoActual || 0
    });
  };
  ...
  return (
    <div className="curso-detalle">
      ...
      <div className="curso-contenedor-principal">
        ...
        {/* El resto del código de la columna derecha permanece igual */}
