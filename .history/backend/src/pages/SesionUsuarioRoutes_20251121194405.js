...
router.get("/semana/:usuarioId", async (req, res) => {
    try {
        ...
        // Obtener sesiones reales desde la BD
        const sesiones = await SesionUsuario.find({
            usuarioId,
            fecha: { $gte: fechaInicio },
        });

        // Normalizar por fecha (sumar horas si hubo varias sesiones en el mismo día)
        const mapa = {};
        ...
        for (let i = 0; i < 7; i++) {
            ...
            resultado.push({
                fecha: fechaISO,
                dia: nombreDia.charAt(0).toUpperCase() + nombreDia.slice(1), // Capitaliza
