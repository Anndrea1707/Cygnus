...
/* ============================================
   🧩 RUTA TEMPORAL PARA ACTUALIZAR CAMPOS
   Ejecutar SOLO UNA VEZ y luego borrar
=============================================== */
router.get("/actualizar-campos/usuarios", async (req, res) => {
  try {
    const usuarios = await Usuario.find();
    let procesados = 0;
    ...
    for (let user of usuarios) {
      ...
      // -------------------------------
      // 1️⃣ nivel_recordacion (encuesta)
      // -------------------------------
      if (
        user.encuesta_inicial?.completada &&
        (user.nivel_recordacion === null || user.nivel_recordacion === undefined)
      ) {
        const t = user.encuesta_inicial.tiempo_area;
        const lambda = user.encuesta_inicial.tasa_olvido;

        if (t != null && lambda != null) {
          user.nivel_recordacion = recuerdo(t, lambda);
          necesitaGuardar = true;
        }
      }

      // -------------------------------
      // 2️⃣ nivel_recordacion_nuevo (cursos)
      // -------------------------------
      const progreso = await ProgresoCurso.findOne({ usuarioId: user._id });

      if (
        (user.nivel_recordacion_nuevo === undefined || user.nivel_recordacion_nuevo === null) &&
        progreso
      ) {
        user.nivel_recordacion_nuevo = null; // se calculará cuando complete un curso
        necesitaGuardar = true;
      }
    ...
    res.json({
      ...
      usuariosActualizados: procesados
    });

  } catch (error) {
    console.error("❌ Error:", error);
    res.status(500).json({ mensaje: "Error actualizando usuarios" });
  }
});
